...
def computeR2(n, dx, a, navg, full_length=True):
    R2 = zeros((n - 1, n))
    for i in range(n):
        c = (i - 0.5) * dx
        d = (i + 0.5) * dx
        ...
        for j in range(n - 1):
            ...
            if (i == 0 or i == n - 1) and not full_length: R2[j, i] *= 2
    return R2

def computeR1L(n, dx, a, navg, full_length=True):
    R1L = zeros((n - 1, n))
    for i in range(n):
        c = (i - 0.5) * dx
        d = (i + 0.5) * dx
        ...
        for j in range(n - 1):
            ...
            if (i == 0 or i == n - 1) and not full_length: R1L[j, i] *= 2
    return R1L
